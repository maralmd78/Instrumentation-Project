...
@app.route('/send')
def send():
    ecg = request.args.get('ecg', 0, type=int)
    heart = request.args.get('heart', 0, type=int)
    ecgs.append(ecg)
    hearts.append(heart)
    return "OK"
...
@app.route('/get_data')
def get_data():
    return jsonify(ecg=ecgs, heart=hearts[-1] if len(hearts)>0 else 0, time=[i/128 for i in range(len(ecgs))])
